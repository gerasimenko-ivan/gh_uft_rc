...
If DoesFolderExist Then
    ...
    Set qtApp = CreateObject("QuickTest.Application") 'Create the Application object
		exitCode = CheckAndLogError(Err, "(line 71)    Set qtApp = CreateObject(""QuickTest.Application"")", logErrorsFilePath)
	...
	qtResultsOpt.ResultsLocation = testResultPath ' Specify the location to save the test results.
		exitCode = CheckAndLogError(Err, "(line 85)	qtResultsOpt.ResultsLocation = testResultPath", logErrorsFilePath)
    qtTest.Run qtResultsOpt, True 'Run the test and wait until end of the test run
		exitCode = CheckAndLogError(Err, "(line 87)    qtTest.Run qtResultsOpt, True", logErrorsFilePath)
...
Sub LogRow(filePath, testPath, startTime, endTime, results)
	...
	If Len(testPath) < 71 Then
		LogToFile filePath, RPad(testPath, " ", 70) & " | " & startTime & " -> " & endTime & " | " & RPad(GetAvailableRAM, "0", 5) & " | " & testResultSymbol & " | " & results
	Else
		line1 = Mid(testPath, 1, 70)
		line2 = Mid(testPath, 71, 70)
		LogToFile filePath, RPad(line1, " ", 70) & " | " & startTime & " -> " & endTime & " | " & RPad(GetAvailableRAM, "0", 5) & " | " & testResultSymbol & " | " & results
...
Function CheckAndLogError(errObject, text, filePath)
	If errObject.Number <> 0 Then
		...
		If errObject.Number = -2147467238 Then
			reasonMessage = "	Highly expected reason: remote computer is no longer connected to local PC."
			LogToFile filePath, reasonMessage
			WScript.Echo reasonMessage
		End If
...
Function GetAvailableRAM()
	...
	For Each objItem in colItems
		Wscript.Echo ""
		Wscript.Echo "RAM usage"
		Wscript.Echo "	Available GB: " & Round(objItem.AvailableBytes / GB,3)
		Wscript.Echo "	CommitLimit GB: " & Round(objItem.CommitLimit / GB,3)
		Wscript.Echo "	Committed GB: " & Round(objItem.CommittedBytes / GB,3)
		Wscript.Echo ""
		
		GetAvailableRAM = Round(objItem.AvailableBytes / GB,3)
	Next
End Function